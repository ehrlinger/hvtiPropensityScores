...
skill (`/codemap hvtiPropensityScores`). If the codemap looks stale, say so and offer to
...
repo itself — `NAMESPACE`, `R/ps-data.R` for the object contract every function returns, and
the README — before editing.

⚠️ The codemap and the vault index this repository under its **repository** name,
`hvtiPropensityScores`, while the package is `hvtiRpropensity`. Searching for the package name
will not find the repo notes.
